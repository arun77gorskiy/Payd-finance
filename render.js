...
(function (global) {
    ...
    //  - есть coingecko_id (или cmc_id)
    //  - last_verified_at в пределах 30 дней
    ...
    async function boot() {
        ...
        if (!PAID.ProjectService || !PAID.DiscoveryService || !PAID.DataProviderFactory) {
            console.error('[PAYD-V2-Render] Application services or factory not available');
            return;
        }
    ...
    async function renderProjects(filter = 'all') {
        ...
        if (!container || !State.projectService) return;
