...
<!-- RESULT MODAL — TABBED, один модуль = один таб со своим уникальным UI -->
...
`;
        ...
        _initChart() {
            ...
            // Подписываемся на изменение видимого диапазона timeScale,
            // чтобы счётчик свечей в HUD (поле "видно: N") обновлялся динамически
            // при зуме/скролле графика.
            if (this._rangeGuardHandler && this.chart && this.chart.timeScale) {
                try { this.chart.timeScale().unsubscribeVisibleTimeRangeChange(this._rangeGuardHandler); } catch (e) {}
                this._rangeGuardHandler = null;
            }
            if (this.chart && this.chart.timeScale && typeof this.chart.timeScale().subscribeVisibleTimeRangeChange === 'function') {
                this._rangeGuardHandler = () => {
                    try {
                        // Throttle: не обновляем HUD чаще, чем 1 раз в 100мс,
                        // чтобы не нагружать DOM при drag-pan.
                        if (this._hudUpdateTimer) return;
                ...
                this.chart.timeScale().subscribeVisibleTimeRangeChange(this._rangeGuardHandler);
            }
        ...
        _updateHud() {
            ...
            if (!hud) {
                console.warn('[LabTrainer] _updateHud: HUD element not found');
                return;
            }
            hud.innerHTML = '';
            const items = [];
            ...
            if (this.currentScenario && this.currentScenario.candles && this.currentScenario.candles.length > 0) {
                ...
                const visibleRange = this.chart && this.chart.timeScale ? (() => {
                    try {
                        const lr = this.chart.timeScale().getVisibleLogicalRange();
                        if (!lr) return null;
                        return Math.max(0, Math.min(candleCount, Math.round(lr.to - lr.from)));
                    } catch (e) { return null; }
                })() : null;

                const visibleText = visibleRange !== null ? ` (видно: ${visibleRange})` : '';
                items.push(`<span class="lt-hud-pill" style="border-color:rgba(212,175,55,0.35);" title="Общее число свечей в сценарии. В скобках — сколько видно сейчас на экране."><span class="lt-hud-label">🕯 CANDLES</span> ${candleCount}${visibleText}</span>`);
                console.log('[LabTrainer] _updateHud: candle counter =', candleCount, visibleText);
            } else {
                console.log('[LabTrainer] _updateHud: no currentScenario or no candles');
            }
        ...
        _showResultScreen(result) {
            ...
            // Краткий вердикт в шапке (одинаков для всех табов)
