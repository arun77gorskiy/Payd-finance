...
with sync_playwright() as p:
    ...
    page.goto("https://0hr40m6rrhdc.space.minimax.io", wait_until="networkidle")
    time.sleep(4)
    ...
    if bbox['y'] < 0 or bbox['y'] > 200:
        ...
        time.sleep(1)
        bbox = page.evaluate("() => document.querySelector('#payd-hero-3d').getBoundingClientRect().toJSON()")
        print(f'Bbox after scroll: {bbox}')
    ...
    if bbox['y'] >= 0 and bbox['y'] + bbox['height'] <= 900:
        page.screenshot(
            path='/workspace/browser/screenshots/hero_v3_final.png',
            clip={'x': bbox['x'], 'y': bbox['y'], 'width': bbox['width'], 'height': bbox['height']}
        )
        print('Saved hero_v3_final.png')
        
        im = Image.open('/workspace/browser/screenshots/hero_v3_final.png')
