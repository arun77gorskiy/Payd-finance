#!/usr/bin/env python3
"""
PAYD INTELLIGENCE V2 — Phase 1.5: DefiLlama Mapping Engine (v3)
================================================================
Improvements over v2:
  1. Name family grouping: when multiple DefiLlama entries share the same
     normalized name (e.g., uniswap-v1, uniswap-v2, uniswap-v3, uniswap-v4
     all normalize to "uniswap"), treat them as a single protocol family.
  2. Within a family, pick the highest-TVL version as the canonical slug.
  3. Add a "version_family" bonus when multiple versions exist.
  4. Smart AMBIGUOUS detection: if all top candidates are in the same family
     (same normalized name), it's NOT ambiguous.
  5. Extra signal: if a project coingeckoId exists but DefiLlama has no
     gecko_id match, do an indirect lookup via symbol+name to suggest the
     "main" version.
"""
...
NOT_APPLICABLE_SECTORS = {"ai", "desci", "rwa", "ZK"}
...
# Bonus when 2+ versions of same protocol are matched
VERSION_FAMILY_BONUS = 0.05
# If name family has only 1 version, no bonus
...
def normalize_name(s):
    ...
    for kw in ["protocol", "network", "labs", "finance", "swap", "exchange",
               "foundation", "io", "app", "dao", "v2", "v3", "v1", "v4", "v5", "v6"]:
        if s.endswith(" " + kw):
            s = s[: -(len(kw) + 1)]
    s = re.sub(r"[^a-z0-9]+", "", s)
    return s
...
def find_candidates(project, idx):
    ...
    if sym in by_symbol:
        for p in sorted(by_symbol[sym], key=lambda x: -(x.get("tvl") or 0))[:8]:
            add(p)

    nm = normalize_name(project.get("name") or project.get("id"))
    if nm in by_name:
        for p in sorted(by_name[nm], key=lambda x: -(x.get("tvl") or 0))[:5]:
            add(p)
...
def map_project(project, idx, weights):
    ...
    for cand in candidates:
        ...
        if conf > 0.3:
            scored.append({"candidate": cand, "confidence": conf, "methods": methods})
    ...
    p_name_norm = normalize_name(project.get("name") or project.get("id"))
    families = defaultdict(list)
    for s in scored:
        c_name_norm = normalize_name(s["candidate"].get("name"))
        families[c_name_norm].append(s)

    # Score each family: take max confidence within family + bonus
    family_scores = []
    for fam_name, members in families.items():
        # Top member of this family
        top = max(members, key=lambda x: x["confidence"])
        fam_conf = top["confidence"]
        if len(members) > 1:
            fam_conf += VERSION_FAMILY_BONUS
        family_scores.append({
            "family_name": fam_name,
            "top": top,
            "members_count": len(members),
            "family_confidence": min(fam_conf, 1.0),
            "all_members": members,
        })

    family_scores.sort(key=lambda x: -x["family_confidence"])
    best_family = family_scores[0]
    best_cand = best_family["top"]["candidate"]
    best_conf = best_family["family_confidence"]
    best_methods = best_family["top"]["methods"]

    # AMBIGUOUS detection:
    # Если 2+ семейств с разными именами имеют conf > 0.80 и разница < 0.05 — AMBIGUOUS
    if len(family_scores) >= 2 and "coingecko" not in best_methods and "twitter" not in best_methods:
        second = family_scores[1]
        # Если семейства имеют РАЗНЫЕ normalized names
        if second["family_name"] != best_family["family_name"]:
            if second["family_confidence"] >= 0.80 and (best_conf - second["family_confidence"]) < 0.05:
                return {
                    "status": "AMBIGUOUS",
                    "defillama_slug": None, "defillama_name": None,
                    "match_confidence": best_conf, "match_method": best_methods,
                    "verified": False,
                    "reason": f"two_distinct_families_within_0.05:{best_family['family_name']}_{second['family_name']}",
                    "alternatives": [
                        {
                            "family": fs["family_name"],
                            "slug": fs["top"]["candidate"].get("slug"),
                            "name": fs["top"]["candidate"].get("name"),
                            "confidence": fs["family_confidence"],
                            "tvl": fs["top"]["candidate"].get("tvl", 0),
                        }
                        for fs in family_scores[:3]
                    ],
                }

    if best_conf >= THRESHOLD_AUTO:
        status = "MAPPED"
    elif best_conf >= THRESHOLD_REVIEW:
    ...
    return {
        ...
        "reason": "auto_match" if status != "NOT_FOUND" else "below_threshold",
        "candidate_tvl": best_cand.get("tvl", 0),
        "candidate_category": best_cand.get("category"),
        "family_name": best_family["family_name"],
        "family_versions": best_family["members_count"],
        "alternatives": [
            {
                "family": fs["family_name"],
                "slug": fs["top"]["candidate"].get("slug"),
...
def main():
    print("=" * 78)
    print("PAYD INTELLIGENCE V2 — Phase 1.5: DefiLlama Mapping (v3)")
    print("=" * 78)
    print()
    ...
    for r in sorted(mp, key=lambda x: -(x["defillama_mapping"].get("candidate_tvl") or 0)):
        ...
        print(f"  {r['project_id']:<28} {r['symbol']:<8} → {m['defillama_slug']:<32} "
              f"tvl={fmt_tvl(m.get('candidate_tvl'))} fam={m.get('family_name')} "
              f"versions={m.get('family_versions')} via {','.join(m['match_method'])}")
    print()
    ...
    for r in sorted(nr, key=lambda x: -x["defillama_mapping"]["match_confidence"]):
        m = r["defillama_mapping"]
        print(f"  {r['project_id']:<28} {r['symbol']:<8} → {m['defillama_slug']:<32} "
              f"conf={m['match_confidence']:.2f} tvl={fmt_tvl(m.get('candidate_tvl'))} "
              f"fam={m.get('family_name')} via {','.join(m['match_method'])}")
    print()
    ...
    for r in am:
        m = r["defillama_mapping"]
        alts = " | ".join([f"{a['family']}(c={a['confidence']:.2f},slug={a['slug']})" for a in m.get("alternatives", [])])
    ...
    for r in sorted(nf, key=lambda x: (x.get("sector") or "", x["project_id"])):
        print(f"  {r['project_id']:<28} {r['symbol']:<8} sector={r['sector']}")
    print()

    # NOT_APPLICABLE summary
    na = [r for r in results if r["defillama_mapping"]["status"] == "NOT_APPLICABLE"]
    print(f"--- NOT_APPLICABLE ({len(na)}) — секторная разбивка ---")
    na_sectors = defaultdict(int)
    for r in na:
        na_sectors[r["sector"]] += 1
    for s, n in sorted(na_sectors.items(), key=lambda x: -x[1]):
        print(f"  {s}: {n}")
    print()

    out_path = OUT_DIR / "defillama_mapping_v3.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            "metadata": {
                ...
                "weights": WEIGHTS,
                "thresholds": {"auto": THRESHOLD_AUTO, "needs_review": THRESHOLD_REVIEW},
                "not_applicable_sectors": sorted(NOT_APPLICABLE_SECTORS),
                "version_family_bonus": VERSION_FAMILY_BONUS,
            },
    ...
    print("=" * 78)
    print("Конец отчёта DefiLlama mapping v3")
    print("=" * 78)
